...
// Dynamischer Import der Map-Komponenten – nur client-side, SSR: false
...
export default function ClubSearch({ initialClubs = [] }: ClubSearchProps) {
  ...
  const filteredAllClubs = useMemo(() => initialClubs.filter(club => {
    const matchText =
  ...
  const filteredClubs = useMemo(() => [...filteredAllClubs].sort((a,b) => (b.rating??0)-(a.rating??0)).slice(0,100), [filteredAllClubs]);
  ...
  const mapCenter: L.LatLngExpression = filteredClubs[0]?.lat && filteredClubs[0]?.lon
  ...
  return (
    <div className="club-search-container">
      <div className="main-content">
        ...
        {/* Map nur client-side */}
        {typeof window !== "undefined" && filteredClubs.some(c=>c.lat&&c.lon) && (
          <MapContainer
